...
<div class="bg-white p-5 rounded-md divide-y">
    ...
    <form action="/pengunjung/update/<?php echo $pengunjung->id; ?>" method="POST" class="mx-auto py-5">
        <input type="hidden" name="_method" value="UPDATE">
        <div class="mb-6">
            <label for="name" class="block mb-2 text-sm text-gray-600">Username</label>
            <input type="text" id="name" name="name" value="<?= $pengunjung->name;?>"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
                required>
        </div>
        <div class="mb-6">
            <label for="name" class="block mb-2 text-sm text-gray-600">Email</label>
            <input type="text" id="name" name="email" value="<?= $pengunjung->email;?>"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
                required>
        </div>
        <div class="mb-6">
            <label for="name" class="block mb-2 text-sm text-gray-600">Telepon</label>
            <input type="text" id="name" name="telepon" value="<?= $pengunjung->telepon;?>"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
                required>
        </div>
        <button type="submit"
                class="w-32 bg-blue-500 text-white py-2 rounded-lg block focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 mt-4 mb-6">Update</button>
    </form>
</div>
